...
(
    function ($, window, document, undefined) {
        ...
        defaults    = {
            ...
            // Executes ONCE when element goes out the TOP (UP) and is
            // completely hidden
            onTopHidden: function () {},

            // Executes ONCE when element comes in from the BOTTOM (DOWN) and
            // is completely in view
            onBottomVisible: function () {},

            // Executes ONCE when element goes out the BOTTOM (UP) and is
            // completely hidden
            onBottomHidden: function () {},

            // Executes ONCE when element comes in from TOP (DOWN) and is
            // completely in view
            onTopVisible: function () {},

            // Executes CONTINUOUSLY when scrolling in either direction WHILE
            // element is in view
            onScroll: function () {},

            // Executes CONTINUOUSLY when scrolling down WHILE element is in
            // view
            ...
            // Offset (constrain the top and bottom of the viewport by a pixel
            // value)
            offset: 0,
            ...
            // Bottom Offset (constrain the bottom of the viewport by a pixel
            // value)
            bottomOffset: 0,
            ...
            parallax: {
                ...
                // Respect the parallax elements margins as a boundary within
                // the parent
                bound: false,
                ...
                // Multiplies the scroll distance by this factor,
                // increasing/decreasing parallax distance
                scalar: 1
            }
        },
        ...
        Plugin.prototype.init = function () {
            ...
            var scroll_obj = {
                ...
                // Handler for scroll direction (regardless if element is in
                // view)
            ...
            var scrollFireHandler = function (e) {
                ...
                var scroll_dir = (
                                 scroll_pos > globals.last_scroll_pos
                                 ) ? 'down' : 'up';

                // Calculate the scroll difference (distance scrolled since
                // last scroll event)
                ...
                $.each(globals.registry, function (index, value) {
                    ...
                    var offset = (
                                     $.isFunction(value._offset)
                                 ) ? value._offset() : value._offset;
                    var topOffset = (
                                        $.isFunction(value._top_offset)
                                    ) ? value._top_offset() : value._top_offset;
                    var bottomOffset = (
                                           $.isFunction(value._bottom_offset)
                                       ) ? value._bottom_offset()
                        : value._bottom_offset;

                    // Reference the target element
                    var elm = $(value._element);

                    // Change our reference during parallax to the container
                    // element instead of selected element
                    ...
                    if (scroll_dir == 'down') {
                        ...
                        // Conditions for when element is PARTIALLY visible in
                        // the viewport (inclusive of offset)
                        var partiallyVisibleTop = scroll_pos <= (
                            elm_pos_top + elm_height - offset
                            );
                        var partiallyVisibleBottom = (
                                                     scroll_pos
                                                     + viewport_height
                                                     ) >= (
                                                     elm_pos_top + offset
                        ...
                        // Conditions for when element is FULLY visible in the
                        // viewport (inclusive of offset)
                        var fullyVisibleTop = scroll_pos <= (
                            elm_pos_top - offset
                            );
                        var fullyVisibleBottom = (
                                                 scroll_pos + viewport_height
                                                 ) >= (
                                                 elm_pos_top + elm_height
                                                 + offset
                                                 );

                        // Conditions stipulating if element is within the
                        // viewport/offset boundaries
                        var partiallyInBounds = (
                        partiallyVisibleTop && partiallyVisibleBottom
                        );
                        var fullyInBounds = (
                        fullyVisibleTop && fullyVisibleBottom
                        );

                        // Conditions for when element begins to go out/come in
                        // the viewport (inclusive of offset)
                        var onTopOut = scroll_pos >= (
                            elm_pos_top - offset
                            );
                        var onBottomIn = (
                                         scroll_pos + viewport_height
                        ...
                        value._on_scroll_down_always(value._element,
                                                     scroll_diff, value
                        );

                        // Fire callbacks that should occur when element is
                        ...
                        if (partiallyInBounds) {
                            ...
                            // Fire callback
                            value._on_scroll_down(value._element, scroll_diff,
                                                  value
                            );
                        }

                        // Calculate point at which element becomes visible
                        // from the bottom
                        var to_top_distance_vis = (
                                                  elm_pos_top + elm_height
                                                  ) - scroll_pos;
                        var perc_from_top_vis = (
                        (
                        to_top_distance_vis + offset
                        ) / viewport_height
                        );
                        perc_from_top_vis = (
                                            perc_from_top_vis > 1
                                            ) ? 1.00
                            : perc_from_top_vis.toFixed(2);

                        // Calculate the point at which the element becomes
                        // hidden at the top
                        var to_top_distance_hid = (
                                                  elm_pos_top + elm_height
                                                  ) - scroll_pos;
                        var perc_from_top_hid = (
                        (
                        to_top_distance_hid - offset
                        ) / viewport_height
                        );
                        perc_from_top_hid = (
                                            perc_from_top_hid > 1
                                            ) ? 1.00
                            : perc_from_top_hid.toFixed(2);

                        // Fire the onBottomVisible callback when element is
                        // completely in view
                        if (perc_from_top_vis < 1 && perc_from_top_hid <= 0
                            && value._on_bottom_visible_once) {
                            value._on_bottom_visible_once = false;
                            value._on_bottom_visible(value._element,
                                                     scroll_diff, value
                            );
                        }

                        // Fire onTopHidden callback when element is completely
                        // out of view
                        if (perc_from_top_hid <= 0 && perc_from_top_vis < 1
                            && value._on_top_hidden_once) {
                            value._on_top_hidden_once = false;
                            value._on_top_hidden(value._element, scroll_diff,
                                                 value
                            );
                        }
                        ...
                        if (value._parallax.active) {
                            initParallax(value._parallax, $(value._element),
                                         offset, scroll_pos, viewport_height
                            );
                        }
                        ...
                        // Conditions for when elements begin to come in/go out
                        // the viewport (inclusive of offset)
                        var onTopIn = scroll_pos <= (
                            elm_pos_bot - offset
                            );
                        var onBottomOut = (
                                          scroll_pos + viewport_height
                        ...
                        if (partiallyInBounds) {
                            ...
                            // Fire callback
                            value._on_scroll_up(value._element, scroll_diff,
                                                value
                            );
                        }
                        ...
                        var to_top_distance_vis = (
                                                  ...
                                                  scroll_pos + elm_height
                                                  + offset
                                                  );
                        var perc_from_top_vis = (
                        to_top_distance_vis / viewport_height
                        );
                        perc_from_top_vis = (
                                            perc_from_top_vis > 1
                                            ) ? 1.00
                        ...
                        // Calculate point at which element becomes hidden at
                        // the bottom
                        var to_top_distance_hid = (
                                                  elm_pos_top + elm_height
                                                  ) - (
                        ...
                        var perc_from_top_hid = (
                        to_top_distance_hid / viewport_height
                        );
                        perc_from_top_hid = (
                                            perc_from_top_hid > 1
                                            ) ? 1.00
                        ...
                        if (perc_from_top_vis >= 0 && perc_from_top_hid >= 1
                            && value._on_top_visible_once) {
                            value._on_top_visible_once = false;
                            value._on_top_visible(value._element, scroll_diff,
                                                  value
                            );
                        }

                        // Fire the onBottomHidden callback when element is
                        // completely out of view
                        if (perc_from_top_hid >= 1 && perc_from_top_vis >= 0
                            && value._on_bottom_hidden_once) {
                            value._on_bottom_hidden_once = false;
                            value._on_bottom_hidden(value._element, scroll_diff,
                                                    value
                            );
                        }
            ...
            // Attach the scroll handler only once to both the scroll and
            // resize events
        ...
        /**
         * Returns an elements margin on the specified side ('top', 'right',
         * 'bottom', 'left').
         * @param elm
         * @param side
         */
        var getElementMargin = function (elm, side) {
            return parseInt(elm.css('margin-' + side
            ).replace(/px|em|%|auto|inherit/g, "")
            );
        };
        ...
        var initParallax = function (
            conf, elm, offset, scroll_pos, viewport_height
        ) {
            ...
            // Percentage distance container is from starting to go in/out of
            // view
            var c_dist = (
                             c_top
                         ) - scroll_pos;
            ...
            var p_pos = (
                        c_height - elm.outerHeight(conf.bound)
                        ) * c_pos;
